...
function createTextTexture(gl: any, text: string, font = "bold 32px system-ui", color = "#ffffff") {
...
class Media {
  ...
  createShader() {
    ...
    // Create text texture for overlay
    const { texture: textTexture } = createTextTexture(this.gl, this.text, this.font, this.textColor);
    ...
    this.program = new Program(this.gl, {
      ...
      fragment: `
        ...
        void main() {
          ...
          float darkOverlay = smoothstep(0.6, 1.0, vUv.y);
          color.rgb = mix(color.rgb, color.rgb * 0.3, darkOverlay * 0.8);
          
          // Text overlay at bottom center
          vec2 textUv = vec2(vUv.x, (vUv.y - 0.7) * 3.0 + 0.5);
          if (textUv.y >= 0.0 && textUv.y <= 1.0 && textUv.x >= 0.0 && textUv.x <= 1.0) {
  ...
  createMesh() {
    ...
    this.plane.onClick = () => {
      if (this.onTeamClick && this.teamData) {
        this.onTeamClick(this.teamData);
      }
    };
  }
  
  createTitle() {
    // Text is now overlaid directly on the image via shader
    // No separate title mesh needed
  }
...
class App {
  ...
  onClick(e: any) {
    if (!this.isDown) {
      ...
      // Simple click detection for centered team
      ...
      const team = this.medias[actualIndex];
      
      if (team && this.onTeamClick) {
        this.onTeamClick(team.teamData);
      }
    }
  }
...
export default function CircularGallery({
  ...
  }, [items, bend, textColor, borderRadius, font, scrollSpeed, scrollEase, onTeamClick]);
